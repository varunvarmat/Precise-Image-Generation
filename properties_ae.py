...
class EncoderLayer(nn.Module):
    def __init__(self, attn, norm1, feed_forward,  norm2, up_sample_linear, dropout=0.1):
        ...
        self.up_sample_linear=up_sample_linear
...
class Encoder(nn.Module):
    def __init__(self, d_model ,dr_ff, N, no_head, schedule, dropout=0.1):
        ...
        self.d_model=d_model 
        self.upsample_schedule=schedule #eg:[32,128]
        assert self.N==len(self.upsample_schedule), "schedule and number of layers should be same"
        for i in range(N):
            self.layers.append(
                EncoderLayer(MultiHeadAttention(self.d_model, no_head, dropout=dropout),
                        nn.LayerNorm(self.d_model),
                        FeedForward(self.d_model, dr_ff, dropout=dropout),
                        nn.LayerNorm(self.d_model),
                        nn.Linear(d_model, self.upsample_schedule[i])
                        ))
        ...
        for i in range(N):
            self.layers.append(
                EncoderLayer(MultiHeadAttention(self.d_model, no_head, dropout=dropout),
                        ...
                        nn.LayerNorm(self.d_model),
                        nn.Linear(d_model, self.downsample_schedule[i])
                        ))
...
class AutoEncoder(nn.module):
    def __init__(self, d_model ,dr_ff, N, no_head, schedule, dropout=0.1):
        ...
        self.down_sample_schedule=schedule[::-2]
        # self.d_model=d_model
